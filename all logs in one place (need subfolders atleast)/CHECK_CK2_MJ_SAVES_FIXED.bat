...
setlocal
set "_CK2_MJ_SELF=%~f0"
set "_CK2_MJ_TMP=%TEMP%\ck2_mj_save_check_%RANDOM%_%RANDOM%.ps1"
echo CK2 Monarch's Journey save check - READ ONLY
echo.
echo This will not edit, rename, load, or delete any save.
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$c=Get-Content -LiteralPath $env:_CK2_MJ_SELF; $i=0; while($i -lt $c.Count -and $c[$i] -ne '# POWERSHELL_PAYLOAD'){$i++}; if($i -ge $c.Count){exit 9}; $p=[string[]]$c[($i+1)..($c.Count-1)]; [IO.File]::WriteAllLines($env:_CK2_MJ_TMP,$p,(New-Object System.Text.UTF8Encoding -ArgumentList $true))"
if errorlevel 1 (
  echo ERROR: Could not prepare the read-only checker.
  pause
  exit /b 1
)
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%_CK2_MJ_TMP%"
set "_CK2_MJ_RC=%ERRORLEVEL%"
del /q "%_CK2_MJ_TMP%" >nul 2>nul
echo.
if not "%_CK2_MJ_RC%"=="0" echo The checker reported an error. Please send a screenshot of this window.
if "%_CK2_MJ_RC%"=="0" echo Send back only CK2_MJ_SAVE_REPORT.txt from your Desktop.
echo.
pause
exit /b %_CK2_MJ_RC%
# POWERSHELL_PAYLOAD
