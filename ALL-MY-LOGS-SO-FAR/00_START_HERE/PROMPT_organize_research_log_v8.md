# MASTER ARCHIVE RULES — structure, tear-down, storage, preservation (v8, self-improving)

**This is the single continuous rulebook.** It preserves v7 without loss and adds the
lessons from the 2026-08-31 V9-polish / DLC-test / 3.3.5.1-port session: direct-session
capture, confidence labels, decision records, reproducible-scratch policy, and
navigation audits. Improving this rulebook is encouraged only through a new version;
never silently rewrite the history or weaken a safety rule.

The earlier instruction files contributed:
...
**Operative status:** v8 supersedes v7 as the text to paste. v1–v7 stay in this folder
as history (they are cited by older reports). If a rule below conflicts with an older
prompt, **this file wins**; if it conflicts with `STATUS.md` /
...
## 4A. Direct-session capture, confidence, and navigation (v8 additions)

A useful session may happen directly in the repository with no uploaded raw transcript.
Do not wait for a later chat export. When the user asks to catalogue the current
session, reconstruct it from the conversation, tool actions, diffs, runtime reports,
and produced artifacts, then create/update the appropriate Part N archive. Add a
`12_raw_chat_logs/INDEX.md` entry saying **direct session; no raw file existed** so the
absence is explicit rather than mistaken for lost evidence.

For every important conclusion, label its evidence class:

- **runtime-proven** — directly observed by the user or a preserved runtime trace;
- **static-proven** — exact bytes, xrefs, symbols, hashes, or reproducible disassembly;
- **strong candidate** — several static clues agree but identity/semantics remain open;
- **hypothesis** — plausible and testable, not evidence;
- **design proposal** — a future architecture, not a claim about existing code.

Never let a proposal harden into a fact merely because later documents repeat it.
Where confidence changes, preserve the earlier model in the evolution/dead-end table
and state what evidence changed it.

When the active direction changes (for example cosmetic polish → DLC test → latest-
version port), record a short **decision entry**: old direction, new direction, user
reason, evidence available, and what remains deferred. This protects side threads from
being mistaken for solved or forgotten work.

**Reproducible scratch policy:** huge objdump files, temporary virtual environments,
function-match caches, decoded intermediates, and one-off probes stay out of Git when
they can be regenerated from a verified artifact. Preserve instead: exact command or
method, input SHA, tool assumptions, all decisive addresses/scores, and the conclusion.
If a custom script becomes necessary to reproduce a claim reliably, promote it to
`05_patches_and_scripts/py/`, document it, and hash it; otherwise remove it.

**Dead-end visibility:** keep dead ends in the owning analysis/archive with the evidence
that closed them. STATUS should contain only a short pointer unless the warning is
operationally dangerous. `BANNED_ARTIFACTS.md` is for harmful tested artifacts;
ordinary disproven theories belong in the archive/analysis, not the banned list.

**Navigation audit after opening a new project branch:** update at least STATUS, PLAN,
archive README read-order/current direction, the owning analysis, and the newest Part N.
Add a CASE only for a stable user-visible behavior; do not turn every experiment into a
case number. Confirm the old goal remains findable and the new goal has one canonical
entry point.

**Self-improvement protocol:** the AI may propose and author a new prompt version when
repeated work exposes a real process gap. It must (1) preserve old versions, (2) list
what changed and why, (3) never weaken hard safety/preservation rules, (4) avoid
project facts that will quickly go stale, and (5) update README/organization history to
name the new operative version. More instructions are not automatically better; add a
rule only when it prevents demonstrated loss, ambiguity, unsafe work, or navigation
cost.
