...
title CK2 MJ V8 clean feat trace

rem This helper is diagnostic only. It does not patch CK2game.exe on disk.
rem Optional arguments:
rem   RUN_MJ_V8_CLEAN_TRACE.bat "C:\path\to\CK2game.exe" "C:\path\to\x64dbg.exe"
...
set "TRACE_SCRIPT=%~dp0MJ_V8_CLEAN_TRACE.txt"
set "EXPECTED_V8=94d6fb403b4541a53f846b348722ee81bc832b66ac853f6fd532f08e2e8b7e93"
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$h=(Get-FileHash -LiteralPath $env:GAME_EXE -Algorithm SHA256).Hash.ToLowerInvariant(); Write-Host ('SHA-256: '+$h); if($h -ne $env:EXPECTED_V8){Write-Host 'ERROR: this is not the confirmed V8 executable. Refusing to attach.' -ForegroundColor Red; exit 3}; Write-Host 'V8 identity: confirmed.' -ForegroundColor Green"
if errorlevel 1 (
  echo.
  echo The clean trace requires the confirmed V8 hash and did not attach.
  pause
  exit /b 3
)
